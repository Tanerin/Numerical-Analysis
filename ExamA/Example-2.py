...
print("Este programa resuelve f(x)=sen(x)+cos(x)-1.2 ")
xr=int(input("Inserte el valor del primer punto: "))
#se obtiene el primer valor
xr=xr-((math.sin(xr)+math.cos(xr)-1.2)/(math.cos(xr)-math.sin(xr)))
...
while True:
    ...
    xra=xr
    xrr.append(xr)
    xrn=xr-((math.sin(xr)+math.cos(xr)-1.2)/(math.cos(xr)-math.sin(xr)))
    #se evalua el error
    e=math.fabs(xra-xrn)
    ve.append(e)
